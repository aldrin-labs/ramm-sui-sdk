...
describe("Sell trade price estimation", () => {
    test("Estimate price of sell trade using, using the SDK", async () => {
        ...
        */
        ...
        ramm.estimatePriceWithAmountIn(estimate_txb2, {
            assetIn: adaType,
            assetOut: dotType,
            amountIn: adaAmount2,
        })
        ...
        const priceEstimationEventJSON2 = devInspectRes2.events[0]
            .parsedJson as PriceEstimationEvent
        ...
        const ada_per_dot_price_est: number =
            ...
            priceEstimationEventJSON.amount_out
        console.log(
            "Estimation: 1 ADA would buy " + dot_per_ada_price_est + " DOT"
        )
        console.log(
            "Estimation: 1 DOT would buy " + ada_per_dot_price_est + " ADA"
        )

        const dot_per_ada_price_est2: number =
            priceEstimationEventJSON2.amount_out /
            priceEstimationEventJSON2.amount_in
        const ada_per_dot_price_est2: number =
            priceEstimationEventJSON2.amount_in /
            priceEstimationEventJSON2.amount_out
        console.log(
            "Second estimation: 1 ADA would buy " +
                dot_per_ada_price_est2 +
                " DOT"
        )
        console.log(
            "Second estimation: 1 DOT would buy " +
                ada_per_dot_price_est2 +
                " ADA"
        )
        ...
        console.log(
            "Actual price: 1 ADA buys " + dot_per_ada_actual_price + " DOT"
        )
        console.log(
            "Actual price: 1 DOT buys " + ada_per_dot_actual_price + " ADA"
        )
        console.log(
